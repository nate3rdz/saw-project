...
export default class PaginationService {
  ...
  static async paginate<
    R extends FindManyOptions<ObjectLiteral>,
  ...
  >(
    ...
    const selectionColumns = columns.filter(
      (column) => !exclude?.includes(column)
    );

    queryBuilder = queryBuilder.select(
      selectionColumns.map((column) => `entity.${column}`)
    );
    ...
    queryBuilder = queryBuilder
      .skip(query.skip || 0)
      .take(query.take || environment.config.pagination.defaultPageSize);
