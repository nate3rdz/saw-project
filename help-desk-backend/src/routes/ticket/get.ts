...
import ITicketsGetInput, { ticketGetInputParamsSchema } from "../../interfaces/endpoints-input/ticket/ITicketGet.js";
...
export class TicketGet extends Endpoint<ITicketsGetInput, Response, ITicketsGetOutput> {
    constructor() {
    ...
    async fn(req: ITicketsGetInput, res: Response): Promise<ITicketsGetOutput> {
        const result = await AppDataSource.getRepository(Tickets).findOne({where: {id: Number(req.params.id)}});
        if (!result)
