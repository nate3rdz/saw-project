...
import ITicketCloseInput, { ticketCloseInputParamsSchema } from "../../interfaces/endpoints-input/ticket/ITicketClose.js";
...
export class TicketClose extends Endpoint<ITicketCloseInput, Response, ITicketManageOutput> {
    constructor() {
    ...
    async fn(req: ITicketManageInput, res: Response): Promise<ITicketManageOutput> {
        const result = await AppDataSource.createQueryBuilder().update(Tickets).set({closed: true}).where("id = :id", {id: req.params.id}).execute();
