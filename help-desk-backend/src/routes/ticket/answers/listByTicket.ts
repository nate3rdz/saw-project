...
import ITicketAnswerListByTicket, { ticketAnswerListByTicketInputParamsSchema } from "../../../interfaces/endpoints-input/ticket/answers/ITicketAnswerListByTicket.js";
...
export class TicketAnswerListByTicket extends Endpoint<ITicketAnswerListByTicket, Response, ITicketAnswerListByTicketOutput> {
    constructor() {
    ...
    async fn(req: ITicketAnswerListByTicket, res: Response): Promise<ITicketAnswerListByTicketOutput> {
        ...
        if (this.innerData.account.id !== ticket.account.id) {
            ...
            if (!hasPermission)
                throw new InternalAPIError('You can\'t list answers of a ticket that wasn\'t created by you.', 403);
        }

        const answers = await AppDataSource.getRepository(TicketAnswers).findAndCount({
