...
import ITicketAnswerInput, { ticketAnswerInputBodySchema, ticketAnswerInputParamsSchema } from "../../../interfaces/endpoints-input/ticket/answers/ITicketAnswer.js";
...
export class TicketAnswer extends Endpoint<ITicketAnswerInput, Response, ITicketAnswerOutput> {
    constructor() {
    ...
    async fn(req: ITicketAnswerInput, res: Response): Promise<ITicketAnswerOutput> {
        const ticket = await AppDataSource.getRepository(Tickets).findOne({where: {id: Number(req.params.id)}, relations: ['account']});
        if (!ticket)
            throw new InternalAPIError('Ticket not found.', 404);

        if (ticket.closed)
            throw new InternalAPIError('The ticket is closed: you can\'t add other answers.', 409);
        ...
        if (this.innerData.account.id !== ticket.account.id) {
            const hasPermission = !!this.innerData.account.permissions.find(element => {
                if (element.name === 'manage-tickets')
                    return true;
            });

            if (!hasPermission)
                throw new InternalAPIError('You can\'t answer a ticket that was not created by you.', 403);
        }
        ...
        const result = await AppDataSource.getRepository(TicketAnswers).insert(answer);
        if (result.identifiers === null)
            throw new InternalAPIError('Error while creating a new answer.', 500);

        return await AppDataSource.getRepository(TicketAnswers).findOne({where: {id: Number(result?.identifiers[0]?.id)}});
    }
}