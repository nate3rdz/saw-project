...
import IUserDeleteInput, { userDeleteInputParamsSchema } from "../../interfaces/endpoints-input/user/IUserDelete.js";
...
export class UserLogin extends Endpoint<IUserDeleteInput, Response, void> {
    ...
    async fn(req: IUserDeleteInput, res: Response): Promise<void> {
       const result = await AppDataSource.createQueryBuilder().delete().from(Accounts).where("id = :id", {id: req.params.id}).execute();
