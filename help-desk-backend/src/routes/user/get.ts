...
import IUsersGetInput, {
  userGetInputParamsSchema,
} from "../../interfaces/endpoints-input/user/IUserGet.js";
...
export class UserGet extends Endpoint<
  IUsersGetInput,
  Response,
  IUserGetOutput
> {
  ...
  async fn(req: IUsersGetInput, res: Response): Promise<IUserGetOutput> {
    ...
    const result = await AppDataSource.getRepository(Accounts).findOne({
      where: { username: req.params.username },
    });
    if (!result) throw new InternalAPIError("Account not found", 404);
