...
import IUserPermissionsUpdateInput, { userPermissionsUpdateInputParamsSchema } from "../../interfaces/endpoints-input/user/IUserPermissionsUpdate.js";
...
export class UserLogin extends Endpoint<IUserPermissionsUpdateInput, Response, void> {
    constructor() {
        super([]);
    ...
    async fn(req: IUserPermissionsUpdateInput, res: Response): Promise<void> {
        const account = await AppDataSource.manager.findOne(Accounts, {where: {id: Number(req.params.id)}});
        if (!account)
            throw new InternalAPIError('Account not found', 404);

        const permissions: Permissions[] = [];
        for (const p of req.body.permissions) {
            const permissionObject: Permissions[] = await AppDataSource.manager.find(Permissions, {where: {name: p}});
        ...
        const account2 = await AppDataSource.manager.find(Accounts, {where: {id: Number(req.params.id)}, relations: {permissions: true} });
    }
}